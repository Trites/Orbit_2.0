...
void OcNode::draw(UWorld *world){
	...
	if (!isLeaf){
		...
		for (OcNode* child : children)
			child->draw(world);
		DrawDebugPoint(world, origin, 16, FColor::Green);

	}
	else{

		FColor renderColor = storedBody != nullptr ? FColor::Red : FColor::Red;
...
void OcNode::insert(ABody* body){
	...
	if (isLeaf){
		...
		if (storedBody == nullptr){
			...
			children[childIndexFor(storedBody->GetActorLocation())]->insert(storedBody);
			storedBody = nullptr;
		}
	}
...
std::size_t OcNode::childIndexFor(const FVector& point) const{
